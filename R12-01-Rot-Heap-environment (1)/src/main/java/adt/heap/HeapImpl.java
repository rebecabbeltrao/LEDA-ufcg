...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		if(!(position <0 || position >= index)){
			int aux = position;
			if(left(position) <= index && getComparator().compare(heap[left(position)], heap[aux])>0){
				aux = left(position);
			}if(right(position)<= index && getComparator().compare(heap[right(position)], heap[aux])>0){
				aux = right(position);
			}
			if(aux != position){
